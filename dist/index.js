...
class CyberSecurityRSSServer {
    ...
    setupToolHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            try {
                switch (name) {
                    case 'list_feeds':
                        return this.handleListFeeds(args);
                    case 'fetch_feed':
                        return this.handleFetchFeed(args);
                    case 'fetch_multiple_feeds':
                        return this.handleFetchMultipleFeeds(args);
                    case 'search_by_keywords':
                        return this.handleSearchByKeywords(args);
                    case 'get_trending_topics':
                        return this.handleGetTrendingTopics(args);
                    case 'get_news_briefs':
                        return this.handleGetNewsBriefs(args);
                    default:
    ...
    sanitizeErrorMessage(error, toolName) {
        ...
        if (error instanceof Error) {
            ...
            if (message.includes('invalid') && (message.includes('keyword') ||
    ...
    logSecurely(level, message, data) {
    ...
    async handleFetchMultipleFeeds(args) {
        ...
        if (feedNames && feedNames.length > 0) {
            feedsToFetch = feedNames.map(name => {
                ...
                if (!feed)
                    throw new Error(`Feed not found: ${name}`);
                return feed;
            });
        }
        else if (category) {
    ...
    async handleSearchByKeywords(args) {
        // Validate keywords
    ...
    async handleGetTrendingTopics(args) {
        // Validate daysBack
    ...
    async handleGetNewsBriefs(args) {
        const category = args.category;
        // Validate date inputs
        let dateFrom;
        let dateTo;
        ...
        const briefs = selectedItems.map(({ item, feedName }) => this.contentSummarizer.generateNewsBrief(item, feedName));
        return {
            content: [
                {
                    type: 'text',
