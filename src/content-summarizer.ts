...
export class ContentSummarizer {
  ...
  generateNewsBrief(item: FeedItem, feedName: string, targetWordCount: number = 120): NewsBrief {
  ...
  generateMultipleBriefs(items: FeedItem[], feedName: string, maxBriefs: number = 10): NewsBrief[] {
    return items
      .slice(0, maxBriefs)
      .map(item => this.generateNewsBrief(item, feedName));
  }

  extractTrendingTopics(allItems: FeedItem[], minMentions: number = 5): TrendingTopic[] {
    const topicMap = new Map<string, { count: number; items: FeedItem[] }>();
  ...
  private extractiveSummarization(text: string, targetWordCount: number): string {
  ...
  private scoreSentence(sentence: string, position: number, totalSentences: number): number {
    ...
    if (position === 0) score += 2; // First sentence
    if (position === totalSentences - 1) score += 1; // Last sentence
  ...
  private categorizeContent(item: FeedItem): string {
  ...
  private getTopicCategory(topicName: string): string {
