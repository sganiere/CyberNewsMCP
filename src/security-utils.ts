...
export class SecurityUtils {
   ...
   */
  static validateUrl(urlString: string): { isValid: boolean; error?: string } {
    try {
  ...
  private static isMetadataEndpoint(hostname: string): boolean {
    const metadataHosts = [
      'metadata.google.internal',
      '169.254.169.254'
    ];
   ...
   */
  static validateKeywords(keywords: string[]): { isValid: boolean; sanitized: string[]; error?: string } {
   ...
   */
  static validateNumber(value: number, min: number = 0, max: number = 1000): { isValid: boolean; error?: string } {
